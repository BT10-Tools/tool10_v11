...
public class MakeSimSetSimilarity {
	...
	private static NodeSimilarity createOneSimilarity(NodeF10 f10, Long entityId1, Long entityId2, String similarityType, String similarityKey,
		...
		try {
			...
			f10.getSimSet().getListSimilarity().add(newSimilarity);
			f10.getSimSet().getMapId2Similarity().put(newSimilarity.getSimilarityId(),newSimilarity);
		} catch (Exception e)	{
	...
	private static void processSimilarityPairs(NodeF10 f10, HashSet<NodePair> pairSet)	{
		...
		System.out.println("processSimilarityPairs f10.getSimSet().getListSimilarity().size():"+f10.getSimSet().getListSimilarity().size());
		for (NodeSimilarity sim : f10.getSimSet().getListSimilarity())	{
	...
	private static void createSimilarities(NodeF10 f10)	{
		...
		MakeSimSetSimilarityAlg.createSimilarities(f10, pairSet);
