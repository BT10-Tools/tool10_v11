...
public class MakeDocSetTanzil {
	...
	public static void createLibraryToken(Connection conn,Connection connT, NodeDocSet library, NodeDoc doc, NodeSentence sentence) {	
		...
		//createSentenceListAndMap(connT, doc.getDocName(), chapter.getChapterName(), sentenceNoList, sentenceMap);
	...
	public static void createLibrarySentence(Connection conn,Connection connT, NodeDocSet library, NodeDoc doc, NodeParagraph paragraph) {	
		//ArrayList<Long> sentenceNoList = new ArrayList<Long>();
		//HashMap<Long,String> sentenceMap = new HashMap<Long,String>();
		...
		for (String token : tokenList)	{
			...
			createLibraryToken(conn,connT, library, doc, sentence);
			cntSentence++;
		}	
	}
	public static void createLibraryParagraph(Connection conn,Connection connT, NodeDocSet library, NodeDoc doc, NodeChapter chapter) {	
		ArrayList<Long> ayetNoList = new ArrayList<Long>();
		HashMap<Long,String> ayetMap = new HashMap<Long,String>();
		
		createAyetListAndMap(connT, doc.getDocName(), chapter.getChapterName(), ayetNoList, ayetMap);
		int cntParagraph = 0;
		for (Long ayetNo : ayetNoList)	{
			...
			createLibrarySentence(conn,connT, library, doc, paragraph);
			cntParagraph++;
		}
	}
	...
	*/
	...
	public static HashMap<NodeToken,ArrayList<String>> createTokens(NodeF10 f10, NodeSentence sentence, ArrayList<String> sentenceStringList) {
		...
		for (String tokenStr : tokenStringList)	{
			...
			NodeToken token = f10.getDocSet().getMapStr2Token().get(tokenStr);
			if (token==null)	{
				...
				token = MakeDocSetTables.createOneToken(f10, null, null, tokenStr, tokenStr+"_desc", (long) tokenStr.length());
			}
	...
	public static HashMap<NodeSentence,ArrayList<String>> createSentences(NodeF10 f10, NodeDoc doc, NodeChapter chapter, NodeParagraph paragraph,
		...
		for (String sentenceStr : sentenceStringList)	{
			...
			String sentenceDesc="Tanzil_doc["+doc.getDocName()+"].chapter["+chapter.getChapterName() + "].paragraph[" + paragraph.getParagraphName()+"]" +
			...
			NodeSentence newSentence = MakeDocSetTables.createOneSentence(f10, paragraph.getParagraphId(), doc.getDocId(), (long) displayOrder, 
	...
	public static HashMap<NodeParagraph,ArrayList<String>> createParagraphs(NodeF10 f10, NodeDoc doc, NodeChapter chapter, NodeSection section,
		...
		for (String str : sectionStringList)	{
			...
			String paragraphDesc="Tanzil_doc["+doc.getDocName()+"].chapter["+chapter.getChapterName() +"].section["+section.getSectionName() + "].paragraph[" + paragraphName+"]";
			...
			NodeParagraph newParagraph = MakeDocSetTables.createOneParagraph(f10, section.getSectionId(), doc.getDocId(), (long) displayOrder, paragraphName, paragraphType, paragraphDesc);
	...
	public static HashMap<NodeSection,ArrayList<String>> createSections(NodeF10 f10, NodeDoc doc, NodeChapter chapter,
		...
		for (String str : chapterStringList)	{
			...
			String sectionDesc="Tanzil_doc["+doc.getDocName()+"].chapter["+chapter.getChapterName() + "].section[" + sectionName+"]";
			...
			NodeSection newSection = MakeDocSetTables.createOneSection(f10, chapter.getChapterId(), doc.getDocId(), (long) displayOrder, sectionName, sectionType, sectionDesc);
	...
	private static ArrayList<ArrayList<String>> getChapters(List<String> docLineList)	{
		ArrayList<ArrayList<String>> chapterList = new ArrayList<ArrayList<String>>();
		if (docLineList==null) return(null);
		System.out.println("MakeDocSetTanzil getChapters docLineList.size():"+docLineList.size());
		...
		for (String lineStr : docLineList)	{
		...
		System.out.println("MakeDocSetTanzil getChapters mapSurahNo2StringBuilder.size():"+mapSurahNo2StringBuilder.size() + "  ,cntInsertedLines:"+cntInsertedLines);
		...
		chapterList.add(sbOthers);  //add the others as the last chapter
 		...
 		System.out.println("MakeDocSetTanzil getChapters chapterList.size():"+chapterList.size());
		return(chapterList);
	}
	public static HashMap<NodeChapter,ArrayList<String>> createChapters(NodeF10 f10, NodeDoc doc, NodeDocBlob docBlob) {
		HashMap<NodeChapter,ArrayList<String>> mapChapter2StringList = new HashMap<>();
		if ((docBlob.getDocBytes()==null) || (docBlob.getDocBytes().length==0)) return(null);
		
		ArrayList<ArrayList<String>> chapterList = getChapters(docBlob.getDocLines());
		...
		for (ArrayList<String> strList : chapterList)	{
			...
			NodeChapter newChapter = MakeDocSetTables.createOneChapter(f10, doc.getDocId(), (long) displayOrder, chapterName, chapterType, chapterDesc);
	...
	public static void processDoc(NodeF10 f10, NodeDoc doc, NodeDocBlob docBlob)	{
		HashMap<NodeChapter,ArrayList<String>> mapChapter2StringList = createChapters(f10,doc,docBlob);
		
		for (NodeChapter chapter : mapChapter2StringList.keySet())	{
			ArrayList<String> chapterStringList = mapChapter2StringList.get(chapter); 
			
			HashMap<NodeSection,ArrayList<String>> mapSection2StringList = createSections(f10,doc, chapter,chapterStringList);
			System.out.println("MakeDocSetTanzil processDoc mapSSection2StringList.size():"+mapSection2StringList.size());
			for (NodeSection section : mapSection2StringList.keySet())	{
				...
				HashMap<NodeParagraph,ArrayList<String>> mapParagraph2StringList = createParagraphs(f10,doc, chapter, section, sectionStringList);
				System.out.println("MakeDocSetTanzil processDoc mapParagraph2StringList.size():"+mapParagraph2StringList.size());
				for (NodeParagraph paragraph : mapParagraph2StringList.keySet())	{
					...
					HashMap<NodeSentence,ArrayList<String>> mapSentence2StringList = createSentences(f10,doc, chapter, paragraph, paragraphStringList);
					System.out.println("MakeDocSetTanzil processDoc mapSentence2StringList.size():"+mapSentence2StringList.size());
