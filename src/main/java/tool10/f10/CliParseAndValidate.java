...
public class CliParseAndValidate {
	...
	private static boolean checkAttachmentName(String str)	{
	...
	if (cmd.hasOption("reverseattachmentname")) 	{	
	    ...
	    if (cmd.hasOption("hash")) 	{	
	    	String str = getYesNo(p,cmd, "hash","yes"); 
	    	if (str==null) return(null); 
	    	p.setHash(str);
    	}
	...
	public static String checkCliParametersImage(CliParameter p, CommandLine cmd) {	
	    //*****
	    if (cmd.hasOption("image")) 	{	String str = cmd.getOptionValue("image"); p.setImage(str);	p.setAction("image");	}
	    else {
	    	return (null);
	    }
	    if (cmd.hasOption("dbname")) 	{	
	    ...
	    if (cmd.hasOption("imagedbname")) 	{	
			String str = cmd.getOptionValue("imagedbname"); 
			if ((str==null) || (str.isEmpty())) {
				System.out.println("Tag Database name is not as expected: database name"+str);
				return(null);
			} 
			p.setImageDbName(str);
		}
	    if (cmd.hasOption("imagedbaction")) 	{	
	    	String str = cmd.getOptionValue("imagedbaction"); 
	    	p.setImageDbAction(str);	
	    	if (str==null) {p.setImageDbAction("newdb");}  //default 
	    	else if ((!"newdb".equals(str)) && (!"renewdb".equals(str)) && (!"newfileset".equals(str)) && (!"appendfile".equals(str)) 
	    			&& (!"usedb".equals(str)))	{
	    		p.setImageDbAction("newdb"); //newdb, renewdb, newfileset, appendfile, usedb
	    	} else {
	    		p.setImageDbAction(str);
	    	}	
	    }
	    ...
	    if (cmd.hasOption("imagesetname")) 	{	
  			String str = cmd.getOptionValue("imagesetname"); 
  			if ((str==null) || (str.isEmpty())) {
  				System.out.println("imagesetname is not as expected: imagesetname "+str);
  				return(null);
  			} 
  			p.setImageSetName(str);
  		}
	    if (cmd.hasOption("tempdir")) 	{	
			String str = cmd.getOptionValue("tempdir"); 
		/*	if (FileUtil.checkDirectoryExists(str))	{
	    ...
	    String postResult = postProcessingImage(p,cmd);
	    if (!"ok".equals(postResult)) return(postResult);
	    return("image");
	}
	...
	private static String postProcessingImage(CliParameter p, CommandLine cmd) {
		return("ok");
	}
	...
	public static String checkAndUpdateAllCliParameters(CliParameter p, CommandLine cmd) {
		...
		action = checkCliParametersLoad(p,cmd);			if ("load".equals(action)) return("ok"); 		
		action = checkCliParametersLoadBlob(p,cmd);		if ("loadblob".equals(action)) return("ok"); 	
		action = checkCliParametersExtract(p,cmd);		if ("extract".equals(action)) return("ok"); 	
		action = checkCliParametersUnzip(p,cmd);		if ("unzip".equals(action)) return("ok"); 		
		action = checkCliParametersUnembed(p,cmd);		if ("unembed".equals(action)) return("ok"); 	
		action = checkCliParametersTransform(p,cmd);	if ("transform".equals(action)) return("ok"); 		
		action = checkCliParametersTag(p,cmd);			if ("tag".equals(action)) return("ok"); 	
		action = checkCliParametersImage(p,cmd);		if ("image".equals(action)) return("ok"); 	
		action = checkCliParametersBook(p,cmd);			if ("book".equals(action)) return("ok"); 	
		action = checkCliParametersSimilarity(p,cmd);	if ("similarity".equals(action)) return("ok");
		return("error");
